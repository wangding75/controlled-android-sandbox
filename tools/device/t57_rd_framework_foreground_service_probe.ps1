param([string]$InstanceName = '', [string]$Serial = '',
      [string]$OutputDirectory = 'build/t57-rd-evidence/framework-foreground-service')

. "$PSScriptRoot/t57_rd_common.ps1"

$caseName = 'RD-10-framework-foreground-service-transport'
$hostPackage = 'com.warden.controlledsandbox.debug'
$guestPackage = 'com.warden.controlledsandbox.fixture'
$activity = 'com.warden.controlledsandbox.fixture.ForegroundServiceKickoffActivity'
...
try {
    ...
    $prepare = Read-CommandResult $device ((Get-Date).AddSeconds(60))
    ...
    $launch = Read-CommandResult $device ((Get-Date).AddSeconds(75))
    ...
    do {
        ...
        $missing = @('FRAMEWORK_FGS_KICKOFF_REQUESTED',
            'GUEST_SERVICE_FRAMEWORK_CREATED',
            'FRAMEWORK_FGS_START_FOREGROUND_PASS' | Where-Object {
    ...
    if ($log -match 'FATAL EXCEPTION|Fatal signal|ANR in|AndroidRuntime|LAUNCH_GATE_FAILED|FRAMEWORK_FGS_REFLECTION_FAILED') {
