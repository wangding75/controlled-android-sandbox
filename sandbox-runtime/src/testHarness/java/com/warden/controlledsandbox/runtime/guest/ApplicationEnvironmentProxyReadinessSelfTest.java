...
public final class ApplicationEnvironmentProxyReadinessSelfTest {
    public static void main(String[] args) {
        Map<String, Boolean> installed = Map.of("userManager", true, "launcherApps", true,
                ...
                "content", true, "settingsIdentity", true, "restrictions", true);
        ...
        boolean restrictionsBlocked = false;
        try {
            ApplicationEnvironmentProxyReadiness.require(
                    Map.of("userManager", true, "launcherApps", true, "shortcut", true,
                            "appWidget", true, "usageStats", true, "content", true,
                            "settingsIdentity", true),
                    profile(VirtualLocationProfileSnapshot.MODE_STATIC));
        } catch (IllegalStateException expected) {
            restrictionsBlocked = expected.getMessage().contains("restrictions");
        }
        require(restrictionsBlocked, "configured application restrictions require the restrictions hook");
    ...
    private static ApplicationEnvironmentProfileSnapshot profile(String mode) {
        return new ApplicationEnvironmentProfileSnapshot(1L, 0L,
                new VirtualUserProfileSnapshot(mode, 0, 100000L, "User", 0, true, true, false,
                        List.of(), List.of("managed"), List.of("true")),
