...
Package and virtual-instance mutations are owned by `PackageManagementService` in the dedicated `:sandbox_package` process. Legacy public role-registration and root-operation AIDL transactions retain their original positions but fail closed. Package Service actively binds the fixed, non-exported `HostPackageAuthorityBootstrapService` and `RuntimePackageAuthorityBootstrapService`; each endpoint returns a process-owned Binder token and its actual process PID. Package Service installs each role with the endpoint UID/PID, allocates the role epoch on the server, links the token to Binder death and revalidates token, UID, PID and epoch on every scoped session operation. Clients cannot choose or advance the authority epoch. `/proc/<pid>/cmdline` and process labels are not authorization inputs.

The Host management and Runtime roles are therefore created through private outbound Binder topology rather than a public same-UID bootstrap race. A Guest process that obtains the root Package Service Binder or even binds a same-package bootstrap component cannot use its capability because its Binder caller PID does not match the endpoint PID fixed at installation. Scoped sessions remain separately bound to their client Binder, owner UID/PID and server authority epoch. Package Service restart discards the slots and repeats the private bootstrap before privileged operations resume.

`GuestContext` keeps its `ContextWrapper` base detached and explicitly implements its Guest-safe surface. Identity-bearing Host operations and known API overloads are denied, while system-service transport is temporarily available only during framework-hook installation. Before Guest application code starts, the service boundary is sealed to the installed-hook snapshot. Missing core Notification, Job, Alarm, Clipboard, Account or Storage hooks block startup; a failed known hook cannot silently return the original Host manager.

This boundary serializes package writes, prevents process-label spoofing, closes public role bootstrap and reduces privileged API exposure. Processes that share one Linux UID still do not form a cryptographic hostile-code boundary: arbitrary untrusted Native code and threats that require kernel-enforced identity separation must use the isolated-UID route.
