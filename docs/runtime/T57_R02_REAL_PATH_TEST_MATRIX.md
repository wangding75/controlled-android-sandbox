...
| RD-03 | system-server `IIntentSender` Binder | `PASS` API32 framework transport reached the real sender route and launched Guest `DetailActivity` |
| RD-04 | framework ServiceManager/AMS service path | `PASS` bounded start/stop path; death/unbind/rebind stress still pending |
| RD-05 | ContentResolver → broker → provider | `PASS` API32 framework transport entered one provider batch transport and returned two typed results |
...
| RD-08 | concurrent Guest processes | `PASS` bounded user0/user1 evidence plus framework transport remote-process evidence |
