...
/** Atomic bounded checksummed persistence for privileged-service profiles. */
final class VirtualPrivilegedServicesStorePersistence {
    ...
    VirtualPrivilegedServicesStorePersistence(File file) {
    ...
    String readPayload() {
        ...
        try {
            if (file.length() > MAX_FILE_BYTES) throw corrupt("PRIVILEGED_SERVICES_FILE_LIMIT_EXCEEDED");
            String text = new String(Files.readAllBytes(file.toPath()), StandardCharsets.UTF_8);
            JSONObject root = new JSONObject(text);
            if (root.optInt("envelopeVersion", -1) != ENVELOPE_VERSION) {
                throw corrupt("PRIVILEGED_SERVICES_ENVELOPE_UNSUPPORTED");
            }
            ...
            if (expected != actual) throw corrupt("PRIVILEGED_SERVICES_CHECKSUM_MISMATCH");
            return payload;
    ...
    void writePayload(String payload) {
        try {
            ...
            if (bytes.length > MAX_FILE_BYTES) throw new IllegalStateException("PRIVILEGED_SERVICES_FILE_LIMIT_EXCEEDED");
            File parent = file.getParentFile();
            if (!parent.isDirectory() && !parent.mkdirs() && !parent.isDirectory()) {
                throw new IllegalStateException("Cannot create privileged-services directory");
            }
            ...
            throw new IllegalStateException("Cannot persist privileged-services profile", error);
        }
    }
    ...
    private static String bounded(String value) {
        if (value == null || value.getBytes(StandardCharsets.UTF_8).length > MAX_PAYLOAD_BYTES) {
            throw new IllegalStateException("PRIVILEGED_SERVICES_PAYLOAD_LIMIT_EXCEEDED");
        }
        return value;
    }
