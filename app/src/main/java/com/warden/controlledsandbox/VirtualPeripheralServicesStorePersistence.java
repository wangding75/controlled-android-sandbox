...
/** Atomic bounded checksummed persistence for peripheral-service profiles. */
final class VirtualPeripheralServicesStorePersistence {
    ...
    VirtualPeripheralServicesStorePersistence(File file) {
    ...
    String readPayload() {
        ...
        try {
            if (file.length() > MAX_FILE_BYTES) {
                throw corrupt("PERIPHERAL_SERVICES_FILE_LIMIT_EXCEEDED");
            }
            String text = new String(Files.readAllBytes(file.toPath()), StandardCharsets.UTF_8);
            JSONObject root = new JSONObject(text);
            if (root.optInt("envelopeVersion", -1) != ENVELOPE_VERSION) {
                throw corrupt("PERIPHERAL_SERVICES_ENVELOPE_UNSUPPORTED");
            }
            ...
            if (expected != actual) {
                throw corrupt("PERIPHERAL_SERVICES_CHECKSUM_MISMATCH");
            }
            return payload;
            ...
            quarantine();
            throw new IllegalStateException("PERIPHERAL_SERVICES_STORE_CORRUPT", error);
        }
    }
    ...
    void writePayload(String payload) {
        try {
            ...
            if (bytes.length > MAX_FILE_BYTES) {
                throw new IllegalStateException("PERIPHERAL_SERVICES_FILE_LIMIT_EXCEEDED");
            }
            File parent = file.getParentFile();
            if (!parent.isDirectory() && !parent.mkdirs() && !parent.isDirectory()) {
                throw new IllegalStateException("Cannot create peripheral-services directory");
            }
            ...
            throw new IllegalStateException("Cannot persist peripheral-services profile", error);
        }
    }
    ...
    private static String bounded(String value) {
        if (value == null || value.getBytes(StandardCharsets.UTF_8).length > MAX_PAYLOAD_BYTES) {
            throw new IllegalStateException("PERIPHERAL_SERVICES_PAYLOAD_LIMIT_EXCEEDED");
        }
        return value;
    }
