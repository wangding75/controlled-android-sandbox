...
/** Typed immutable Binder page for VirtualJobSnapshot. */
public final class VirtualJobPage implements Parcelable, VirtualPageView<VirtualJobSnapshot> {
    private final List<VirtualJobSnapshot> items;
    private final List<VirtualPageBlob> blobs;
    private final String nextPageToken;
    private final long snapshotRevision;

    public VirtualJobPage(List<VirtualJobSnapshot> items, List<VirtualPageBlob> blobs,
    ...
    private VirtualJobPage(Parcel in) {
        this(in.createTypedArrayList(VirtualJobSnapshot.CREATOR),
                in.createTypedArrayList(VirtualPageBlob.CREATOR), in.readString(), in.readLong());
    }
    public List<VirtualJobSnapshot> items() { return items; }
    ...
    public static final Creator<VirtualJobPage> CREATOR = new Creator<>() {
        @Override public VirtualJobPage createFromParcel(Parcel in) { return new VirtualJobPage(in); }
        @Override public VirtualJobPage[] newArray(int size) { return new VirtualJobPage[size]; }
    };
}
