...
/** Typed immutable Binder page for VirtualAccountSummary. */
public final class VirtualAccountPage implements Parcelable, VirtualPageView<VirtualAccountSummary> {
    private final List<VirtualAccountSummary> items;
    private final List<VirtualPageBlob> blobs;
    private final String nextPageToken;
    private final long snapshotRevision;

    public VirtualAccountPage(List<VirtualAccountSummary> items, List<VirtualPageBlob> blobs,
    ...
    private VirtualAccountPage(Parcel in) {
        this(in.createTypedArrayList(VirtualAccountSummary.CREATOR),
                in.createTypedArrayList(VirtualPageBlob.CREATOR), in.readString(), in.readLong());
    }
    public List<VirtualAccountSummary> items() { return items; }
    ...
    public static final Creator<VirtualAccountPage> CREATOR = new Creator<>() {
        @Override public VirtualAccountPage createFromParcel(Parcel in) { return new VirtualAccountPage(in); }
        @Override public VirtualAccountPage[] newArray(int size) { return new VirtualAccountPage[size]; }
    };
}
