...
import json
for item in json.load(open('build-environment.lock.json'))['deviceTestBuild']['artifacts']:
...
OUT="$ROOT/artifacts/m5-device-test-build/$COMMIT"
rm -rf "$OUT"
python3 scripts/verify-device-test-artifacts.py --android-tools --output "$OUT"
printf 'PASS M5 device-test APK build: %s\n' "$OUT"
