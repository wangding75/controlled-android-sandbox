...
& javac --release 17 -d (Join-Path $Build 'domain') $Domain
& java -cp (Join-Path $Build 'domain') com.warden.controlledsandbox.domain.SelfTest
& javac --release 17 -d (Join-Path $Build 'wrapper') (Join-Path $Root 'tools\wrapper-src\org\gradle\wrapper\GradleWrapperMain.java')
& jar --create --file (Join-Path $Root 'gradle\wrapper\gradle-wrapper.jar') -C (Join-Path $Build 'wrapper') .
Get-ChildItem $Root -Recurse -Filter *.xml | ForEach-Object { [xml](Get-Content $_.FullName -Raw) | Out-Null }
