...
try {
    ...
    $Tasks += 'check'
    $Tasks += @($Lock.deviceLabBuild.artifacts | ForEach-Object { $_.gradleTask })
    ...
    $Out = Join-Path $Root "artifacts\m5-device-lab-build\$Commit"
    Remove-Item $Out -Recurse -Force -ErrorAction SilentlyContinue
    & python scripts\verify-device-test-artifacts.py --profile device-lab --android-tools --output $Out
    if ($LASTEXITCODE -ne 0) { throw 'Device-lab APK verification failed' }
    Write-Host "PASS M5 device-lab APK build: $Out"
