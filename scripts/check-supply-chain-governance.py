...
def validate_workflow_text(text: str, lock: dict) -> list[str]:
    ...
    gate_command = "python3 scripts/check-supply-chain-governance.py"
...
def main() -> int:
    ...
    report = {
        "task": "source-supply-chain-governance",
        "finding": "Immutable CI, Gradle dependency verification, and commit identity governance",
    ...
    output = ROOT / "build" / "verification" / "source-supply-chain-governance.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    if errors:
        print("FAIL source supply-chain governance", file=sys.stderr)
        for error in errors:
            print(" - " + error, file=sys.stderr)
        return 1
    print(
        "PASS source supply-chain governance "
