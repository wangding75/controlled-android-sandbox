...
session_aidl=(ROOT/'sandbox-contract/src/main/aidl/com/warden/controlledsandbox/contract/IPackageManagementSession.aidl').read_text(encoding='utf-8-sig')
root_aidl=(ROOT/'sandbox-contract/src/main/aidl/com/warden/controlledsandbox/contract/IPackageService.aidl').read_text(encoding='utf-8-sig')
...
service_source=(ROOT/'app/src/main/java/com/warden/controlledsandbox/PackageServiceBinder.java').read_text(encoding='utf-8-sig')
management_source=(ROOT/'app/src/main/java/com/warden/controlledsandbox/PackageManagementSession.java').read_text(encoding='utf-8-sig')
authority_guard=(ROOT/'app/src/main/java/com/warden/controlledsandbox/PackageManagementAuthorityGuard.java').read_text(encoding='utf-8-sig')
...
verifier_source=(ROOT/'app/src/main/java/com/warden/controlledsandbox/PackageCallerVerifier.java').read_text(encoding='utf-8-sig')
...
for path in (ROOT/'app/src/main/java').rglob('*.java'):
    ...
    if 'new SandboxPackageLifecycle' in path.read_text(encoding='utf-8-sig'):
        errors.append(f'{path.relative_to(ROOT)} directly constructs SandboxPackageLifecycle')

compiler=(ROOT/'tools/static_android_compile.py').read_text(encoding='utf-8-sig')
