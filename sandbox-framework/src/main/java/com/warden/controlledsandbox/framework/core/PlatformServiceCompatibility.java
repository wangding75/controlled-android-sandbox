...
public final class PlatformServiceCompatibility {
    ...
    /**
     * Android 12L/API32 and later expose these Binder names only to the system API domain.  An
     * ordinary application process cannot discover them through ServiceManager on the supported
     * AOSP/Google images, even though the services are registered for system components.  Each caller must
    ...
    public static String restrictionReason(String logicalCapability) {
        ...
        if ("wifiScanner".equals(logicalCapability)) {
            return "EXPECTED_PLATFORM_BEHAVIOR:API32+ system-api wifiscanner is not discoverable by "
                    + "the untrusted application domain; WifiManager remains the supported app API";
        }
        if ("dnsResolver".equals(logicalCapability)) {
            return "EXPECTED_PLATFORM_BEHAVIOR:API32+ system-api dnsresolver is not discoverable by "
